...
const NotificationPage: NextPageWithLayout = () => {
    ...
    const handleGetNotifications = async (
        ...
        try {
            const { status, payload, pagination } =
                await getNotificationsWithPage(currentPage, limit, query);
            if (status === 200) {
        ...
        <section className="scrollHidden relative flex flex-col items-start w-full h-full px-5 pb-5 pt-5 overflow-auto gap-5">
            ...
            <div className="w-full pb-10">
                <div className="flex items-center gap-2">
                    {tagsNotification.map(
                        (tag: TagNotification, index: number) => (
                            <button
                                key={index}
