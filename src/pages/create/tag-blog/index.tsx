...
import { ICreateTagBlog, ISelectItem, ITagBlog } from "~/interface";
...
const CreateCategoryPage: NextPageWithLayout = () => {
  ...
  const [data, setData] = useState<ICreateTagBlog>(initData);
  ...
  const uploadThumbnail = async (source: File) => {
    if (source) {
      ...
      try {
        const { status, payload } = await uploadTagBlogImage(formData);
  ...
  const handleOnSubmit = async () => {
    ...
    try {
      const payload = await createTagBlog(sendData);
      ...
      toast.error("Error in create tag", {
        position: toast.POSITION.TOP_RIGHT,
      });
      setLoading(false);
    }
